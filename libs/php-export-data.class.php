...
class ExportDataExcel extends ExportData {
	...
	function generateHeader() {
		...
		$output .= "<Style ss:ID=\"Default\" ss:Name=\"Normal\"><Alignment ss:Vertical=\"Bottom\"/><Borders/><Font ss:FontName=\"Calibri\" x:Family=\"Swiss\" ss:Size=\"12\" ss:Color=\"#000000\"/><Interior/><NumberFormat/><Protection/></Style>";
	    $output .= "<Style ss:ID=\"s62\"><Font ss:FontName=\"Calibri\" x:Family=\"Swiss\" ss:Size=\"12\" ss:Color=\"#DD0806\"/></Style>";
		$output .= "</Styles>\n";
	...
	private function generateCell($item) {
		...
		$output .= "            ";
		if ($type == 'Number') {
			$celFloat = floatval($item);
			$output .= ($celFloat < 0)? "<Cell ss:StyleID=\"s62\">" : "<Cell>";
		} else if (preg_match("/^-/i", $item)) {
			$output .= "<Cell ss:StyleID=\"s62\">";
		} else {
